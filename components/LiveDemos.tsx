...
import { Maximize2, RefreshCw, ChevronRight, ExternalLink, X, Layout, AlertCircle, CheckCircle2, Link, Zap } from 'lucide-react';
              ...
              {imageErrors[selectedDemo.id] ? (
                ...
                >
                  <div className="p-10 border-2 border-dashed border-white/10 flex flex-col items-center glass-2">
                    ...
                    <p className="text-[9px] font-mono text-white/20 uppercase max-w-xs">NEURAL LINK BROKEN. ASSET "{selectedDemo.image.substring(1)}" NOT FOUND IN PUBLIC FOLDER.</p>
                  </div>
                </motion.div>
              ) : (
                  ...
                  {!loadedImages[selectedDemo.id] && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black z-30">
                       <div className="flex flex-col items-center gap-4">
                          <RefreshCw size={32} className="text-[#CCFF00] animate-spin" />
                          <span className="text-[10px] font-mono font-black text-[#CCFF00] tracking-widest">CONNECTING_TO_ASSET...</span>
                       </div>
                    </div>
                  )}
                  <motion.img 
